...
import {FilterObj} from '../models/callLog';
...
export const USTimeToTime: Record<string, string> = {
  '12 AM': '00',
  '1 AM': '01',
  '2 AM': '02',
  '3 AM': '03',
...
export const getFilter = (active: string, direction: string, size: number, page: number, filterValue: string, date1: any, date2: any, keys: string[], val?: string, customerFilter?: any) => {
  let filter = 'filter='
  let obj: FilterObj = {
    limit: size,
    skip: (page - 1) * size
  }
  let sobj: Record<string, any> = {}
  let rkeys: any = []
  let created = {}
  ...
  if (filterValue) {
    ...
    if (obj.where) {
      if (obj.where.and) {
        ...
        obj = { ...obj, 'where': {and: _.map(obj.where, (value:any, key:string) => ({[key]: value})).concat(customerFilter)}};
      }
    } else {
...
export const getCountWhere = (filterValue: string, date1: string, date2: string, keys: string[], customerFilter?: any) => {
  let obj: FilterObj = {
  }
  let sobj: Record<string, any> = {}
  let rkeys = []
